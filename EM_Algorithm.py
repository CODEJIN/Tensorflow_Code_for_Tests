...
def EM_Algoritm_Discret_Data(sample, mixture_Count=10, iteration= 10000):
    '''
    Searcing the Gaussian-mixture by Expectation-Maximization algorithm
    Args:
    ...
    with tf.variable_scope('em_Algorithm'): 
        ...
        x_Range = tf.range(tf.cast(tf.shape(sample)[0], dtype=tf.float32))  #[L]
        ...
        def while_Body(index, alpha_Tensor, mu_Tensor, sigma_Tensor):
            ...
            w_MT /= tf.reduce_sum(w_MT, axis=0);    #[M, L]
                
            alpha_Tensor = tf.reduce_sum(tiled_Sample * w_MT, axis=1) / tf.reduce_sum(sample, axis=0);
            mu_Tensor = tf.reduce_sum(tiled_X_Range * tiled_Sample * w_MT, axis=1) / tf.reduce_sum(tiled_Sample * w_MT, axis=1);
            sigma_Tensor = tf.clip_by_value(
                tf.sqrt(tf.reduce_sum(tiled_Sample * w_MT * tf.pow(tiled_X_Range - tiled_Mu_Tensor, 2), axis=1) / tf.reduce_sum(tiled_Sample * w_MT, axis=1)),
                clip_value_min=0.1,
...
def Batch_EM_Algoritm_Discret_Data(sample, mixture_Count=10, iteration= 10000):
    '''
    Searcing the Gaussian-mixture by Expectation-Maximization algorithm
    Args:
        sample: 2d tensor (BATCHxL). The 'L' mean 'x' axis. Each cell is how much sample is in the point. Total sample count 'N' is 'sum(range(sample) * sample)'.
        mixture_Count: positive integer scalar(M).
        iteration: positive integer scalar.
    Returns:        
        alpha: 2D tensor representing the weights of Gaussian. Size is (BATCHxM)
        mu: 2D tensor representing the loc of Gaussian. Size is (BATCHxM)
        sigma: 2D tensor representing the std of Gaussian. Size is (BATCHxM)
    Refer:
        https://stackoverflow.com/questions/22750634/expectation-maximizationgmm-em-never-finds-the-correct-parameters-mixture-of
        https://www.tensorflow.org/api_docs/python/tf/distributions/Normal        
    '''
    with tf.variable_scope('em_Algorithm'):
        ...
        alpha_Tensor = tf.ones([batch_Size, mixture_Count], dtype=tf.float32);
        mu_Tensor = tf.random_uniform([batch_Size, mixture_Count], dtype=tf.float32);
        sigma_Tensor = tf.ones([batch_Size, mixture_Count], dtype=tf.float32) * tf.reduce_sum(sample, axis=1, keepdims=True) / 100;   #When sigma is too small, the values become nan.

        x_Range = tf.tile(
            tf.expand_dims(tf.range(tf.cast(tf.shape(sample)[1], dtype=tf.float32)), axis=0),
        ...
        def while_Body(index, alpha_Tensor, mu_Tensor, sigma_Tensor):
            tiled_Sample = tf.tile(tf.expand_dims(sample, axis=1), multiples=(1, mixture_Count, 1))    #[Batch, M, L]
            tiled_X_Range = tf.tile(tf.expand_dims(x_Range, axis=1), multiples=(1, mixture_Count, 1))    #[Batch, M, L]   the value is index.
            tiled_Mu_Tensor = tf.tile(tf.expand_dims(mu_Tensor, axis=2), multiples=(1, 1, tf.shape(sample)[1]))    #[Batch, M, L]
            tiled_Sigma_Tensor = tf.tile(tf.expand_dims(sigma_Tensor, axis=2), multiples=(1, 1, tf.shape(sample)[1]))    #[Batch, M, L]
    
            w_MT = tf.exp(-0.5 * tf.pow(tiled_X_Range - tiled_Mu_Tensor, 2) / tf.pow(tiled_Sigma_Tensor, 2)) / tf.sqrt(2 * math.pi * tf.pow(tiled_Sigma_Tensor, 2))  #[Batch, M, L]
            w_MT /= tf.reduce_sum(w_MT, axis=1, keepdims=True); #[Batch, M, L]
    
            alpha_Tensor = tf.reduce_sum(tiled_Sample * w_MT, axis=2) / tf.reduce_sum(sample, axis=1, keepdims=True);   #[Batch, M]       
            mu_Tensor = tf.reduce_sum(tiled_X_Range * tiled_Sample * w_MT, axis=2) / tf.reduce_sum(tiled_Sample * w_MT, axis=2);    #[Batch, M]       
            sigma_Tensor = tf.clip_by_value(
                tf.sqrt(tf.reduce_sum(tiled_Sample * w_MT * tf.pow(tiled_X_Range - tiled_Mu_Tensor, 2), axis=2) / tf.reduce_sum(tiled_Sample * w_MT, axis=2)),
                clip_value_min=0.1,
                clip_value_max=np.inf
                )   #[Batch, M]
